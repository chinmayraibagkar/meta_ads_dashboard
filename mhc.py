    ...
    if 'yesterday_data' not in st.session_state:
        with st.spinner('Fetching data for the first time...'):
            ...
            for acnt in accounts:
                df = request_report(acnt, "last_30d", 800)
                if df is not None:
                    dfs.append(df)
            
            # Combine all dataframes
            st.session_state.last_month_data = pd.concat(dfs, ignore_index=True)
            
            # Convert Reporting starts column to datetime if not already
            st.session_state.last_month_data['Reporting starts'] = pd.to_datetime(st.session_state.last_month_data['Reporting starts'])
        ...
        try:
            ...
            with tab2:
                ...
                with col2:
                    ...
                    date2_start = st.date_input("Select Start Date 2", datetime.now() - timedelta(days=60))
                    date2_end = st.date_input("Select End Date 2", datetime.now() - timedelta(days=30))
            ...
            with tab3:
                ...
                account_names = st.session_state.last_month_data['Account name'].unique()
                selected_accounts = st.multiselect("Select Account(s)", account_names, key="account_selection")

                campaign_filter = st.session_state.last_month_data['Account name'].isin(selected_accounts) if selected_accounts else True
                campaign_names = st.session_state.last_month_data[campaign_filter]['Campaign name'].unique()
                selected_campaigns = st.multiselect("Select Campaign(s)", campaign_names, key="campaign_selection")
